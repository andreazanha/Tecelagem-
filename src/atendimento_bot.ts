...
export const ATEND_COLUNAS = [
  ...
  { id: "efetuou-pedido", label: "✅ Efetuou pedido", cor: "#059669" },
...
// Consumidor final (varejo): a Big NÃO vende no varejo — indica uma LOJA PARCEIRA da
// região. Sem estado/cidade, pergunta o estado; depois manda o link da vitrine (ou os
// cards das lojas). NÃO fala de preço nem passa pro vendedor.
...
export async function processar(conv0: Conversa, texto: string, deps: Deps): Promise<Resultado> {
  ...
  switch (conv.estado) {
    ...
    case "aguardando-cnpj": {
      ...
      // Poucos dígitos / "não tenho CNPJ" → consumidor final: indica loja parceira da região.
      if (digitos.length < 8) {
