...
push.post("/subscribe", async (c) => {
  const b = await c.req
    .json<{ endpoint?: string; keys?: { p256dh?: string; auth?: string }; usuario?: string }>()
    .catch(() => ({}) as { endpoint?: string; keys?: { p256dh?: string; auth?: string }; usuario?: string });
  if (!b.endpoint || !b.keys?.p256dh || !b.keys?.auth) return c.json({ error: "inscrição inválida" }, 400);
  const usuario = String(b.usuario ?? "").trim() || null;
  await c.env.DB.prepare(
    `INSERT INTO push_subscriptions (endpoint, p256dh, auth, user_agent, usuario) VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(endpoint) DO UPDATE SET p256dh = excluded.p256dh, auth = excluded.auth, user_agent = excluded.user_agent, usuario = excluded.usuario`
  )
    .bind(b.endpoint, b.keys.p256dh, b.keys.auth, c.req.header("user-agent") || "", usuario)
    .run();
