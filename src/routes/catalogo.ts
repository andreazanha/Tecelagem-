...
modelos.get("/", async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT nome, parte, ref, composicao, tassel_peseira, tassel_almofada, etiqueta_colar, pronta_entrega FROM modelos ORDER BY nome"
  ).all();
...
modelos.post("/", async (c) => {
  const b = await c.req.json<{
    ...
    pronta_entrega?: number | boolean; // modelo é PRONTA ENTREGA (ex.: Manta Lumi), além dos "KIT"
  ...
  const pe = b.pronta_entrega ? 1 : 0; // modelo é pronta entrega
  ...
  const stmts: D1PreparedStatement[] = [
    c.env.DB.prepare(
      `INSERT INTO modelos (nome, parte, ref, composicao, tassel_peseira, tassel_almofada, etiqueta_colar, pronta_entrega)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(nome) DO UPDATE SET
         ...
         etiqueta_colar = excluded.etiqueta_colar, pronta_entrega = excluded.pronta_entrega`
    ).bind(nome, parte, b.ref || null, b.composicao || null, tp, ta, ec, pe),
  ];
  if (de && de !== nome) {
...
modelos.get("/:nome", async (c) => {
  ...
  const m = await c.env.DB.prepare(
    "SELECT nome, parte, ref, composicao, tassel_peseira, tassel_almofada, etiqueta_colar, pronta_entrega FROM modelos WHERE nome = ?"
  ).bind(nome).first();
