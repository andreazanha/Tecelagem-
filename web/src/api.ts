...
export const api = {
  ...
  atendRespostas: () =>
    fetch("/api/atendimento/respostas").then((r) => j<{ titulo: string; texto: string }[]>(r)),
  atendSalvarRespostas: (respostas: { titulo: string; texto: string }[]) =>
    jsonPost("/api/atendimento/respostas", respostas).then((r) => j<{ ok: boolean; respostas: { titulo: string; texto: string }[] }>(r)),
