...
function GalgaChip({ c }: { c: CardProducao }) {
  const g = galgaDe(c);
  return <span className={"tp-gchip g" + g}>galga {g}</span>;
}
...
function PainelSepPE({ cfg, cards, onAbrir, onAcao, onEntrada }: {
  ...
  // Número do card:
  //  • desmembrado (tem op) → mostra o NÚMERO ORIGINAL do pedido (op), não o código pai.
  //  • explosão ainda inteira (vários números) → código pai ou "1º +N" (não a lista toda).
...
function PainelTec({
  ...
  const verbo = cfg.fazerLabel.toLowerCase(); // "tecer" | "passar"
  const gerundio = cfg.fazendoLabel; // "Tecendo" | "Passando"
  ...
  if (cfg.painelPessoas) {
    ...
    const filaPE = cards.filter((c) => c.pe_tipo === "separado"); // kits do estoque (separado)
    const filaUne = cards.filter((c) => !!c.une_pe); // pedidos misto que se unem ao kit (junto)
    ...
    return (
      <div className="painel">
        ...
        {cfg.painelPE && (
          <div className="tp-tiles dois" style={{ marginBottom: 18 }}>
            <button className="tp-tile pe" onClick={() => onAbrirFila({ pe: "separado" })}>
              <span className="tp-ic">📦</span>
              <span className="tp-body">
                <span className="tp-n">{filaPE.length}</span>
                <span className="tp-l">Pronta entrega</span>
                <span className="tp-sub">produtos saem do estoque · enviar à parte</span>
              </span>
              <span className="tp-go">ver fila ›</span>
            </button>
            <button className="tp-tile mix" onClick={() => onAbrirFila({ pe: "junto" })}>
              <span className="tp-ic">🔗</span>
              <span className="tp-body">
                <span className="tp-n">{filaUne.length}</span>
                <span className="tp-l">Vão unir com pronta entrega</span>
                <span className="tp-sub">produção + kit do estoque (junto)</span>
              </span>
              <span className="tp-go">ver fila ›</span>
            </button>
          </div>
  ...
  const subFila = (arr: CardProducao[]) => {
    const n = crit(arr);
    return (n ? `${n} crítico${n > 1 ? "s" : ""} · ` : "") + `na fila p/ ${verbo}`;
  };
  return (
    <div className="painel">
      <div className="tp-tiles quatro">
      ...
      <div className="tp-sec">{gerundio} agora</div>
      <div className="tp-cols">
        <PainelCol titulo="Galga 3" cls="g3" gerundio={gerundio} cards={colG3} onAbrir={onAbrir} />
        <PainelCol titulo="Galga 7" cls="g7" gerundio={gerundio} cards={colG7} onAbrir={onAbrir} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="tp-col">
      <div className={"tp-colh " + cls}>
        ...
        <span className="tp-colc">{cards.length} {gerundio.toLowerCase()}</span>
      </div>
...
// Fila aberta pelos botões grandes: por galga (Tecer), todas as galgas (Passar) ou envio.
function FilaModal({
  cfg,
  galga,
  envio,
                  ...
                  {darEntrada ? (
                    ...
                    <div className="fila-acts">
                      {pe === "junto" && c.status !== "fazendo" ? (
                        ...
                        <button className="fila-btn go" onClick={() => onAcao(c, "enviar")}>Enviar p/ {proxNome} ▸</button>
                      )}
                    </div>
                  ) : pessoa || defeito ? (
                    <div className="fila-acts">
