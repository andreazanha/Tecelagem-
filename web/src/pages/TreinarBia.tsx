...
export function TreinarBia() {
  ...
  async function salvar() {
    ...
    try { await api.salvarConhecimento(form); setForm(null); setMsg("✓ Salvo! A Big já vai usar."); carregar(); setTimeout(() => setMsg(""), 2500); }
  ...
  return (
    <div className="pagina" style={{ maxWidth: 860, margin: "0 auto", padding: "0 12px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, margin: "10px 0 6px" }}>
        <div>
          <h1 style={{ margin: 0 }}>🎓 Treinar a Big</h1>
          <p className="muted" style={{ margin: "2px 0 0" }}>Cadastre perguntas comuns e a resposta certa. A Big usa isso pra responder direitinho.</p>
        </div>
      ...
      <div style={{ background: "#faf5ff", border: "1px solid #e9d5ff", color: "#4b3f63", borderRadius: 10, padding: "10px 13px", fontSize: 12.5, margin: "8px 0 16px", lineHeight: 1.5 }}>
        💡 Escreva a <b>pergunta como o cliente faria</b> e a <b>resposta que a Big deve dar</b>. Ela adapta ao tom da conversa. Ideal pra: pedido mínimo, prazos, formas de pagamento, private label, dúvidas frequentes. Não invente preço aqui se não quiser que ela fale — pode mandar ela chamar o vendedor.
      </div>
      ...
      {form && (
        <div style={{ border: "2px solid #8b5cf6", borderRadius: 12, padding: 16, marginBottom: 18, background: "#fffdff", color: "#1e293b" }}>
          ...
          <label className="campo"><span className="campo-label">Resposta que a Big deve dar</span>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11.5, fontWeight: 800, padding: "3px 9px", borderRadius: 999, background: i.ativo ? "#dcfce7" : "#f1f5f9", color: i.ativo ? "#166534" : "#64748b", whiteSpace: "nowrap" }}>
                    {i.ativo ? "🟢 Ativa — a Big responde" : "⚪ Inativa — a Big ignora"}
                  </span>
                </div>
