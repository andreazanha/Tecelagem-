...
export function RelatorioVendas() {
  ...
  function carregar() { setCarregando(true); api.relatorioVendas(de || undefined, ate || undefined).then(setDados).catch(() => setDados(null)).finally(() => setCarregando(false)); }
        ...
        <>
          ...
          <div className="card pad" style={{ marginBottom: 14, background: "var(--accent-soft, #eef)" }}>
            ...
            <div className="row-gap" style={{ gap: 18, flexWrap: "wrap", fontSize: 14 }}>
              {champModelo && <div><div className="muted" style={{ fontSize: 11.5 }}>Modelo mais vendido</div><strong>{champModelo.produto}</strong> · {pc(champModelo.pecas)} pç</div>}
              ...
              {champCombo && <div><div className="muted" style={{ fontSize: 11.5 }}>Combinação campeã</div><strong>{champCombo.produto}</strong> · {champCombo.cor} · {champCombo.tamanho} · {pc(champCombo.pecas)} pç</div>}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 14 }}>
                ...
                {(verTodos ? modelos : modelos.slice(0, 12)).map((m) => {
                  ...
                  return (
                    <div key={m.produto} onClick={() => setModelo(m.produto)} style={{ display: "grid", gridTemplateColumns: "minmax(90px,1.4fr) 3fr auto", alignItems: "center", gap: 10, cursor: "pointer", padding: "2px 4px", borderRadius: 6, background: sel ? "var(--accent-soft, #eef)" : undefined }}>
                      <span style={{ fontWeight: sel ? 800 : 600, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={m.produto}>{m.produto}</span>
                      <div style={{ background: "var(--line)", borderRadius: 6, height: 16, overflow: "hidden" }}><div style={{ width: (v / max * 100).toFixed(1) + "%", height: "100%", background: "var(--accent)", borderRadius: 6, minWidth: v > 0 ? 3 : 0 }} /></div>
                      <span style={{ fontWeight: 700, fontVariantNumeric: "tabular-nums", fontSize: 13, textAlign: "right", minWidth: 64 }}>{metrica === "valor" ? rs(v) : pc(v)}</span>
                    </div>
                  );
            ...
            <div className="card pad">
              <div style={{ marginBottom: 8 }}><strong>{modeloSel || "—"}</strong> {cardMod[0] && <span className="muted" style={{ fontSize: 13 }}>· {pc(cardMod[0].pecas)} pç · {rs(cardMod[0].valor)} · {cardMod[0].cores} cores · {cardMod[0].tamanhos} tamanhos</span>}</div>
              ...
              <table className="table" style={{ minWidth: 520 }}>
                ...
                <tbody>
                  {dados.combos.slice(0, 20).map((c, i) => (
                    <tr key={i}><td className="muted">{i + 1}</td><td className="strong">{c.produto}</td><td>{c.cor}</td><td>{c.tamanho}</td><td className="num" style={{ fontWeight: 700 }}>{pc(c.pecas)}</td><td className="num">{rs(c.valor)}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
